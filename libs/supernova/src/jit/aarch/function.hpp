...
#include <vector>
#include <optional>
namespace supernova::jit::aarch64
{
    struct XmmWrapper : asmjit::a64::Xmm
    {
        bool is64bit = false;
        explicit XmmWrapper(const asmjit::a64::Xmm& value):  asmjit::a64::Xmm(value)
        {
            this->is64bit = false;
        }
        explicit XmmWrapper(const asmjit::a64::Xmm& value, bool is64bit):  asmjit::a64::Xmm(value), is64bit(is64bit)
        {
        }
    };
    struct function_builder
    {
        std::unordered_map<long long, asmjit::a64::Mem> i64_consts;
        std::unordered_map<int, asmjit::a64::Mem> i32_consts;
        std::unordered_map<short, asmjit::a64::Mem> i16_consts;
        std::unordered_map<char, asmjit::a64::Mem> i8_consts;
        std::unordered_map<float, asmjit::a64::Mem> f32_consts;
        std::unordered_map<double, asmjit::a64::Mem> f64_consts;
        std::shared_ptr<asmjit::JitRuntime> rt;
        std::shared_ptr<asmjit::CodeHolder> code;
        std::shared_ptr<asmjit::a64::Compiler> co;
        ...
        static std::shared_ptr<function_builder> create(const std::shared_ptr<asmjit::JitRuntime>& rt,
                                                        ...
                                                        const bool& logger = false);
        [[nodiscard]] asmjit::a64::Gp i8() const
        {
            return co->newInt8();
        }
        [[nodiscard]] asmjit::a64::Gp i8(const char val)
        {
            asmjit::a64::Gp initialized_gp = i8();
            move(initialized_gp, i8_const(val), true);
            return initialized_gp;
        }
        [[nodiscard]] asmjit::a64::Mem i8_const(const char val)
        {
            if (not i8_consts.contains(val))
                i8_consts[val] = co->newByteConst(asmjit::ConstPoolScope::kLocal, val);
            return i8_consts[val];
        }
        [[nodiscard]] asmjit::a64::Gp i16() const
        {
            return co->newInt16();
        }
        [[nodiscard]] asmjit::a64::Gp i16(const short val)
        {
            asmjit::a64::Gp initialized_gp = i16();
            move(initialized_gp, i16_const(val), true);
            return initialized_gp;
        }
        [[nodiscard]] asmjit::a64::Mem i16_const(const short val)
        {
        ...
        [[nodiscard]] asmjit::a64::Mem f32_const(const float val)
        {
        ...
        [[nodiscard]] asmjit::a64::Mem f64_const(const double val)
        {
            if (not f64_consts.contains(val))
                f64_consts[val] = co->newDoubleConst(asmjit::ConstPoolScope::kLocal, val);
            return f64_consts[val];
        }
        [[nodiscard]] asmjit::a64::Gp i32() const
        {
            return co->newInt32();
        }
        [[nodiscard]] asmjit::a64::Gp i32(const int val)
        {
            asmjit::a64::Gp initialized_gp = i32();
            move(initialized_gp, i32_const(val));
            return initialized_gp;
        }
        [[nodiscard]] asmjit::a64::Mem i32_const(const int val)
        {
            if (not i32_consts.contains(val))
                i32_consts[val] = co->newInt32Const(asmjit::ConstPoolScope::kLocal, val);
            return i32_consts[val];
        }
        [[nodiscard]] asmjit::a64::Gp i32fromI8or16(const asmjit::a64::Gp& i8or16) const
        {
        ...
        [[nodiscard]] asmjit::a64::Gp i64() const
        {
            return co->newInt64();
        }
        [[nodiscard]] asmjit::a64::Gp i64(const long long val)
        {
            asmjit::a64::Gp initialized_gp = i64();
            move(initialized_gp, i64_const(val));
            return initialized_gp;
        }
        [[nodiscard]] asmjit::a64::Mem i64_const(const long long val)
        {
        ...
        void fetch_argument(const size_t index, const asmjit::a64::Reg& r, const bool f64 = false) const
        {
        ...
        void add(const asmjit::a64::Gp& r, const asmjit::a64::Gp& lhs, const asmjit::a64::Gp& rhs) const // NOLINT
        {
        ...
        void sub(const asmjit::a64::Gp& r, const asmjit::a64::Gp& lhs, const asmjit::a64::Gp& rhs) const //NOLINT
        {
            // use i32 for byte or word
            if (rhs.isGpb() or rhs.isGpw())
            {
        ...
        void mul(const asmjit::a64::Gp& r, const asmjit::a64::Gp& lhs, const asmjit::a64::Gp& rhs) const // NOLINT
        {
        ...
        void div(const asmjit::a64::Gp& r, const asmjit::a64::Gp& lhs, const asmjit::a64::Gp& rhs) const // NOLINT
        {
            // use i32 for byte or word
            if (rhs.isGpb() or rhs.isGpw())
            {
            ...
            else
            {
                ...
                co->idiv(r, lhs, rhs);
                if (r.isGp64())
                    co->mov(r, asmjit::a64::rax);
                else
                    co->mov(r, asmjit::a64::eax);
            }
        }
        void mod(const asmjit::a64::Gp& r, const asmjit::a64::Gp& lhs, const asmjit::a64::Gp& rhs) const // NOLINT
        {
            // use i32 for byte or word
            if (rhs.isGpb() or rhs.isGpw())
            {
        ...
        void increment(const asmjit::a64::Gp& r) const // NOLINT
        {
            // use i32 for byte or word
        ...
        void decrement(const asmjit::a64::Gp& r) const // NOLINT
        {
            // use i32 for byte or word
            if (r.isGpb() or r.isGpw())
            {
        ...
        void return_value(const asmjit::a64::Gp& r) const
        {
        ...
        void compare(const asmjit::a64::Gp& lhs, const asmjit::a64::Gp& rhs) const
        {
            co->cmp(lhs, rhs);
        }
        ...
        void jump_equal(const asmjit::a64::Gp& t1, const asmjit::a64::Gp& t2, const asmjit::Label& l) const
        {
            compare(t1, t2);
            co->je(l);
        }
        ...
        void jump_not_equal(const asmjit::a64::Gp& t1, const asmjit::a64::Gp& t2, const asmjit::Label& l) const
        {
            compare(t1, t2);
            co->jne(l);
        }
        ...
        void jump_lower(const asmjit::a64::Gp& t1, const asmjit::a64::Gp& t2, const asmjit::Label& l) const
        {
            compare(t1, t2);
            co->jl(l);
        }
        ...
        void jump_lower_equal(const asmjit::a64::Gp& t1, const asmjit::a64::Gp& t2, const asmjit::Label& l) const
        {
            compare(t1, t2);
            co->jle(l);
        }
        ...
        void jump_greater(const asmjit::a64::Gp& t1, const asmjit::a64::Gp& t2, const asmjit::Label& l) const
        {
            compare(t1, t2);
            co->jg(l);
        }
        ...
        void jump_greater_equal(const asmjit::a64::Gp& t1, const asmjit::a64::Gp& t2, const asmjit::Label& l) const
        {
            compare(t1, t2);
            co->jge(l);
        }
        ...
        void move(const asmjit::a64::Gp& to, const asmjit::a64::Gp& from, const bool is_char = false) const
        {
            if (is_char)
                co->movzx(to, from);
            else
        ...
        void move(const asmjit::a64::Gp& to, const asmjit::a64::Mem& from, const bool is_char = false) const
        {
            if (is_char)
                co->movzx(to, from);
            else
        ...
        void move(const asmjit::a64::Mem & to, const asmjit::a64::Gp& from) const
        {
            co->mov(to, from);
        }
        ...
        void move(const XmmWrapper& to, const asmjit::a64::Mem& from) const
        {
            if (to.is64bit)
                co->movsd(to, from);
            else
        ...
        void move(const asmjit::a64::Mem& to, const XmmWrapper& from) const
        {
            if (from.is64bit)
                co->movsd(to, from);
            else
        ...
        void call(const asmjit::Imm& func, const asmjit::FuncSignature& signature,
                  const std::vector<asmjit::a64::Reg>& args, const std::optional<asmjit::a64::Reg>& return_to) const
        {
            ...
            for (int i = 0; i < args.size(); i++)
            {
                if (not(args[i].isGp64() or args[i].isGp32() or args[i].isGpb() or args[i].isXmm()))
                    throw std::invalid_argument(fmt::format("unsupported callee argument type for arg[{}]: {}", i,
                                                            static_cast<int>(
                                                                asmjit::a64::Reg::typeIdOf(args[i].type()))));
                if (args[i].isGpb() and signature.arg(i) != asmjit::TypeId::kInt8 or args[i].isGp64() and signature.
                    ...
                    throw std::invalid_argument(fmt::format(
                        ...
                        static_cast<int>(asmjit::a64::Reg::typeIdOf(args[i].type())),
            ...
            if (return_to)
            {
                ...
                if (return_to->isGp32() and signature.ret() != asmjit::TypeId::kInt32 or return_to->isXmm() and
                    ...
                    throw std::invalid_argument(fmt::format("incorrect callee return type for {}, expected: {}",
                                                            static_cast<int>(asmjit::a64::Reg::typeIdOf(
                                                                return_to->type())),
                                                            static_cast<int>(signature.ret())));
                                                                      ...
                                                                      const bool& logger)
    {
        ...
        func->co = std::make_unique<asmjit::a64::Compiler>(&*func->code);
        if (logger)
